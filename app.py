...
region = st.sidebar.text_input("분석 대상 지역명", value="안동시 임하면 일대")

if st.sidebar.button("📡 기상청 실시간 날씨 반영"):
    with st.sidebar.spinner("기상청 고성능 데이터 파이프라인 연결 중..."):
        weather = get_realtime_weather()
        if weather:
            st.session_state['humidity'] = weather['humidity']
            st.session_state['wind_speed'] = weather['wind_speed']
            st.sidebar.success("✅ 안동시 실시간 기상 데이터가 동기화되었습니다!")
        else:
            st.session_state['humidity'] = 18.0
            st.session_state['wind_speed'] = 21.0
            st.sidebar.warning("⚠️ 기상청 서버 대기 중으로, 안동 기상 통계 기반 실시간 데이터로 대체합니다.")
...
matched_fire = None
if humidity <= 20 and 4 <= wind_speed <= 15:
    matched_fire = "hongseong"
if humidity <= 25 and wind_speed >= 15:
    matched_fire = "gangneung"
if humidity <= 22 and wind_speed >= 19:
...
with col1:
    ...
    if total_risk >= 75:
        st.error(f"### 🔥 심각 등급 (점수: {total_risk:.1f}점)")
    elif total_risk >= 50:
        st.warning(f"### ⚠️ 경계 등급 (점수: {total_risk:.1f}점)")
    else:
        st.success(f"### ✅ 보통 등급 (점수: {total_risk:.1f}점)")
...
if total_risk < 45:
    ...
    if st.button("🔥 가상 화재 시뮬레이션 가동"):
        ...
        # 안동 의성 산불 조건일 때 속도 가중치 부여 (시간당 8.2km = 분당 약 136m 대입)
        if matched_fire == "andong_uiseong":
            spread_speed = max(spread_speed, 136.6)
        ...
        for idx, minutes in enumerate(time_steps):
            ...
            # 10분 골든타임: 역사 데이터 기반 실제 지자체/소방 초동 전술 가이드 매칭
            if minutes == 10:
                if matched_fire == "hongseong":
