    ...
    try:
        ...
        if res.status_code == 200 and 'response' in res.json():
            ...
            for item in items:
                ...
                elif item['category'] == 'VEC': live_wd = float(item['obsrValue']) # 🌟 풍향 각도 추출 추가!
    except: pass
    return live_t, live_h, live_w, live_wd

# 🌟 풍향 각도(0~360도)를 소방관들이 쓰는 한국어 8방위 문자열 및 불길이 밀려가는 '최전방 확산 위험 방위'로 환산하는 물리 함수
...
if st.sidebar.button("🛰️ 해당 구역 실시간 감시 파이프라인 가동", type="primary"):
    if region_input.strip() != "":
        ...
        with st.sidebar.spinner("기상청 방재 기상 관측망(AWS) 실시간 통신 중..."):
            ...
            st.session_state['wd_val'] = real_wd # 실시간 풍향 세션 주입 완료
...
# 🌟 사용자가 시연장이나 발표 중에 바람 방향을 자유자재로 돌려볼 수 있는 풍향 각도 조절 슬라이더 추가 배치!
...
# 🌟 풍향 한국어 명칭 및 불길이 밀려오는 최전방 타깃 방위 획득
...
with col_status:
    ...
    if final_area_score >= 0.15:
        ...
        if is_night:
            # 🌟 [v10.0 전술 고도화] 부제목과 3단 대책에 바람 방향에 따른 최전방 대피령 타깃 방위를 다이렉트로 결합 기재!!
